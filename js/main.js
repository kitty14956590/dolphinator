...
function rgb(red,green,blue,text) {
	// I don't like this but it said I cant have octal escape in format stirng :c
...
try {
	if(args.encode) {
		encode = args.encode;
		process.stdout.write("\033[38;2;196;0;255m");
		iterate_thingy(encode,function(i) {
			process.stdout.write(encode.charCodeAt(i).toString(2).padStart(16,"0").replace(/1/g,"e").replace(/0/g,"E"));
		});
		process.stdout.write("\033[38;2;255;255;255m");
		console.log();
	} else if(args.decode) {
		var decode = args.decode.match(/.{1,16}/g);
		process.stdout.write("\033[38;2;196;0;255m");
		iterate_thingy(decode, function(i){
			process.stdout.write(String.fromCharCode(parseInt(decode[i].replace(/e/g,"1").replace(/E/g,"0"),2)));
		});
		process.stdout.write("\033[38;2;255;255;255m");
		console.log();
	} else {
